...
frameCount = 1#used to average table
while(cap.isOpened()):
    ret, frame = cap.read()
    ...
    if(frameCount < NUM_TABLE_FRAMES_AVERAGED):
        newBox = getTable(hsv.copy(),THRESHOLD=90000)
        box = averageBox(box,newBox,frameCount)
        frameCount = frameCount +1
    
    #This sets up frame for ball searching
    hsv = rotateFrame(box,hsv.copy())    #This should return the hsv with the table isolated
    outFrame = rotateFrame(box,frame.copy())
    mask = getHSVMask(hsv.copy(),threshold=8000)#This does some light dilation, prevents some crash for some reason
    ...
    #testFrame = cv2.cvtColor(hsv,cv2.COLOR_HSV2BGR)
